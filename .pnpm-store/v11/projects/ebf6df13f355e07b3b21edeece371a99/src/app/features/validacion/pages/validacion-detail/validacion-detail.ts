...
export class ValidacionDetail implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (publicId) this.loadRevision(publicId);
  }
  ...
  approveRevision(): void {
    ...
    this.validacionService.resolveRevision(currentRevision.id, 5).subscribe({
      next: () => {
        this.loadRevision(currentRevision.id);
        this.showStatusNotice('La planeación fue aprobada correctamente.', 'success');
      }
    });
  }
  ...
  requestCorrections(): void {
    ...
    this.validacionService.resolveRevision(currentRevision.id, 4).subscribe({
      next: () => {
        this.loadRevision(currentRevision.id);
        this.showStatusNotice('Se solicitaron correcciones al docente.', 'warning');
      }
    });
  }
  ...
  addComment(comment: string): void {
    ...
    this.validacionService.addComment(currentRevision.id, comment).subscribe({
      next: () => {
        this.loadRevision(currentRevision.id);
        this.showStatusNotice('Comentario agregado correctamente.', 'success');
      }
    });
  }
  ...
  private loadRevision(publicId: string): void {
    this.validacionService.getRevisionById(publicId).subscribe({
