...
export class PlaneacionesService {
  ...
  private toUnit(
    ...
    return {
      ...
      evaluaciones,
      apertura: this.toActivities(dto.secuencias, 'apertura'),
      desarrollo: this.toActivities(dto.secuencias, 'desarrollo'),
      cierre: this.toActivities(dto.secuencias, 'cierre'),
      referencias
    };
  }
  ...
  private toActivities(
    ...
    return items
      ...
      .map((item, index) => ({
        ...
        metodoTecnica: this.strategyLabel(phase, item.estrategia),
  ...
  private toEditDto(
    ...
    return {
      ...
      unidades: planeacion.formulario.unidades.map((unit, index) => {
        const originalUnit = original.unidades.find(item => item.id === unit.id);

        return {
          id: originalUnit?.id ?? null,
          ...
          temas: unit.temas.map((topic, topicIndex) => ({
            id: originalUnit?.temas.some(item => item.id === topic.id)
              ? topic.id
              : null,
            tema: topic.tema,
          ...
          secuencias: this.toSequenceDtos(
            unit,
            originalUnit?.secuencias ?? []
          )
        };
      }),
      referencias: references.map((reference, index) => ({
        id: original.referencias.some(item => item.id === reference.id)
          ? reference.id
          : null,
  ...
  private toEvaluationDtos(
    ...
    return unit.evaluaciones.flatMap(evaluation => {
      const original = originals.find(item => item.id === evaluation.id);
      ...
      return types.map((type, typeIndex) => {
        ...
        return {
          id: typeIndex === 0 ? original?.id ?? null : null,
  ...
  private toSequenceDtos(
    ...
    return (['apertura', 'desarrollo', 'cierre'] as FaseSecuencia[])
      .flatMap(phase => unit[phase].map(activity => {
        const original = originals.find(item => item.id === activity.id);
        const strategy = this.strategyValue(phase, activity.metodoTecnica) ??
          original?.estrategia;

        if (!strategy) {
          throw new Error(`La estrategia "${activity.metodoTecnica}" no existe en el backend.`);
        }
  ...
  getSeguimientoDirectivo(): Observable<SeguimientoPlaneacion[]> {
    return of(
      this.planeaciones.map(item => {
        const fechaLimite = item.fechaLimiteCaptura;
        const diasRestantes = fechaLimite
          ? this.getDaysRemaining(fechaLimite)
          : undefined;
